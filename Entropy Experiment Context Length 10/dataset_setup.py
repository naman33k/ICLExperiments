...
class DatasetManager:
    ...
    def setup_mixed_dataset(self) -> Tuple[DataLoader, Any]:
        ...
        # Get the fixed dictionary from config
        dictionary = self.config.get_dictionary()
        
        # Create mixed dataset with the fixed dictionary
    ...
    def create_mixed_dataset(self) -> Any:
        """Create and return the mixed dataset (for getting the dictionary)."""
        l = self.config.context_length
        vocab_size = self.config.vocab_size
        ...
        # Create mixed dataset with the fixed dictionary
        mixed_dataset = datasets.KVRetrievalDatasetMixedDictNewIdea(
            'train', 
            vocab_size=vocab_size, 
            length=l, 
            dictionary=dictionary,
    ...
    def create_fixed_dataset_loader(self, dictionary: Any = None) -> DataLoader:
        """Create a fixed dictionary dataset loader for evaluation."""
        # Use dictionary from config if none provided
